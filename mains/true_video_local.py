...
def push_stream_process(pre, video_clip_np, putStream):
    ...
    while cur_frame < video_clip_np.shape[0]:
        ...
        print(video_clip_np[cur_frame].shape)
        putStream.push(video_clip_np[cur_frame])
        print("pushing cur_frame: ", cur_frame)
        cur_frame += 1
    # video_clip_list = list(video_clip_np)
    # putStream.push(video_clip_list)
...
def VideoRealtimeDetection(model, file_path, npy_save_dir=None, resize=(224, 224), frame_window=150, frame_interval=75,
# def VideoRealtimeDetection(model, file_path, npy_save_dir=None, resize=(640, 384), frame_window=150, frame_interval=75,
                           target_frames=75, sample='uniform_sampling', gap=2):
    # Load video
    ...
    height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)) #360
    ...
    while (cap.isOpened()) and idx<len_frames:
        ...
        if len(frames_queue)==frame_interval:
            ...
            video_frame_res += ([1] * (idx - pre_idx) if pred == 1 else [0] * (idx - pre_idx))
            pre_idx = pre_idx + frame_interval
            frames_queue = frames_queue[frame_interval:]

            #对图像进行画框、加字、pushStream等操作-------------------------------------
            # print("push stream with video_clip_np length : ", video_clip_np.shape[0], "pre is :", pred)
            # po.apply_async(push_stream_process, (pred, file_path, video_clip_np))
            # t = threading.Thread(target=push_stream_process, args=(pred, file_path, video_clip_np))
            # print("process will start")
            # t.start()
            push_stream_process(pred, file_path, video_clip_np)
